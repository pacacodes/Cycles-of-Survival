#!/usr/bin/env node
/**
 * Copies vines organism card PNGs from the working organisms output folder
...
function parseArgs() {
  const opts = {
    ...
    categoryConfig: 'game/config/organisms.vines.json',
    workingDir: 'output/Working Organism Cards/current',
    outDir: 'output/Working Organism Cards/Vines',
    safeDir: 'saved_files/Working Organism Cards/Vines',
    legacyDir: 'Output/Vines',
  };
...
function main() {
  ...
  console.log(`✅ Copied ${copiedCount}/${totalCategoryOrgs} vines organism cards`);
  if (missingCount > 0) {
