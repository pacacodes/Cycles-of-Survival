...
module.exports = function drawEffectField(ctx, { textX, rowY, scale, color, titleColor, main, sub, maxWidth, hexToRgba, drawTop, drawBottom }) {
  ...
  const bottomH = Math.round(8 * scale * mainLineCount); // 8px per line of main text
  ...
  // Draw main text (may wrap to multiple lines)
  ctx.font = `bold ${fieldSize}px "DejaVu Sans", sans-serif`;
  ctx.fillText(labelPart, textX, rowY);
  
  // Wrap main text if needed
  if (mainLineCount > 1 && mainAvailWidth) {
    ...
    for (let i = 0; i < words.length; i++) {
      ...
      if (testW > mainAvailWidth && currentLineText) {
        // Current line is full, draw it
        ctx.fillText(currentLineText, textX + labelPartW, currentY);
        currentLineText = word;
        currentY += fieldSize + lineGap;
      } else {
