...
// Draws the neon floodplain badge PNG centered in a circular placeholder
module.exports = async function floodplainBiome(ctx, x, y, { width, height, scale }) {
  ctx.save();
  const badgeRadius = Math.min(width, height) * 0.5 * 0.9;
  const centerX = x + width / 2;
  const centerY = y + height / 2;
  // Placeholder: draw an orange circle with 'FL' text
  try {
    const imgPath = path.join(__dirname, '../../badges/biomes/Floodplain.png');
