...
const DNA_LENGTH = DNA_GRID_COLUMNS * DNA_GRID_ROWS;
const DNA_PAD_SOURCE_TARGET = 4000;
const FRONT_DNA_OVERLAY_COLOR = 'rgba(168, 180, 168, 0.14167500)';
...
function getDnaDisplaySequence(dna, card) {
  ...
  if (!isExtinctAnimal(card)) {
    return dna.replace(/N/g, '').slice(0, DNA_LENGTH);
  }
  ...
  return dna.slice(0, DNA_LENGTH).padEnd(DNA_LENGTH, 'N');
}

module.exports = function drawFrontDnaOverlay(ctx, x, y, scale, card) {
  const safeInset = BLEED * scale;
  const trimX = x + safeInset;
  const trimY = y + safeInset;
  ...
  const cellW = trimW / grid.cols;
  const cellH = trimH / grid.rows;
  const fontSize = Math.max(1, Math.floor(Math.min(cellW, cellH) * 0.9));

  ctx.save();
  ctx.fillStyle = FRONT_DNA_OVERLAY_COLOR;
  ...
  for (let row = 0; row < grid.rows; row++) {
    for (let col = 0; col < grid.cols; col++) {
      ...
      const cx = trimX + (col * cellW) + (cellW / 2);
      const cy = trimY + (row * cellH) + (cellH / 2);
