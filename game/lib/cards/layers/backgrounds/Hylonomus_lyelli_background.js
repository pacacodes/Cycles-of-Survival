...
// Hylonomus lyelli (Early reptile) background renderer for Card 49
module.exports = async function hylonomusLyelliBackground(ctx, x, y, { width, height, corner, scale, roundedRectPath }) {
  ...
  // Overlay the Hylonomus lyelli PNG
  const { loadImage } = require('canvas');
  const imgPath = path.resolve(__dirname, '../main-photo/Hylonomus lyelli.png');
  if (fs.existsSync(imgPath)) {
    try {
      ...
      const imgX = x + (width - imgW) / 2 - 80;
      const imgY = y + colorBlockHeight + 40 - 10 - 260;
      ctx.save();
      ctx.globalAlpha = 1.0;
