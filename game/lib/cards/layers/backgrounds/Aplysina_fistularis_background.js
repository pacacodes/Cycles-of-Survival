...
// Aplysina fistularis (Tube sponge) background renderer for Card 39
module.exports = async function aplysinaFistularisBackground(ctx, x, y, { width, height, corner, scale, roundedRectPath }) {
  ...
  // Overlay the Aplysina fistularis PNG
  const { loadImage } = require('canvas');
  const imgPath = path.resolve(__dirname, '../main-photo/Aplysina fistularis.png');
  if (fs.existsSync(imgPath)) {
    try {
      ...
      const scaleFactor = 1.32 * 0.90;
      const imgW = width * scaleFactor;
      const imgH = (height - colorBlockHeight) * scaleFactor;
      const imgX = x + (width - imgW) / 2 - 60;
      const imgY = y + colorBlockHeight - 120;
      ctx.save();
      ctx.globalAlpha = 1.0;
