...
// Morganucodon watsoni (Morganucodon) background renderer for Card 67
module.exports = async function morganucodonWatsoniBackground(ctx, x, y, { width, height, corner, scale, roundedRectPath }) {
  ...
  // Overlay the Morganucodon watsoni PNG
  const { loadImage } = require('canvas');
  const imgPath = path.resolve(__dirname, '../main-photo/Morganucodon watsoni.png');
  if (fs.existsSync(imgPath)) {
    try {
      ...
      const imgY = y + colorBlockHeight + 40 - 140;
      ctx.save();
      ctx.globalAlpha = 1.0;
