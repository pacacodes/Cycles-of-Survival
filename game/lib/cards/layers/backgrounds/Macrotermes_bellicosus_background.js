...
// Macrotermes bellicosus (Termite) background renderer for Card 86
module.exports = async function macrotermesBellicosusBackground(ctx, x, y, { width, height, corner, scale, roundedRectPath }) {
  ...
  // Overlay the Macrotermes bellicosus PNG
  const { loadImage } = require('canvas');
  const imgPath = path.resolve(__dirname, '../main-photo/Macrotermes bellicosus.png');
  if (fs.existsSync(imgPath)) {
    try {
      ...
      const imgY = y + colorBlockHeight + 50;
      ctx.save();
      ctx.globalAlpha = 1.0;
