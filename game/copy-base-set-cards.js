#!/usr/bin/env node
/**
 * Copies base-set organism card PNGs from the working organisms output folder
...
(async function main() {
  try {
    ...
    if (missing.length > 0) {
      console.warn(`⚠ Could not copy ${missing.length} base-set card(s) — run generate:organisms:working first:`);
      for (const m of missing) console.warn(`  - ${m}`);
    }

    console.log(`✓ Copied ${expectedFiles.size} base-set cards to ${outDir}`);
    console.log(`✓ Mirrored to ${safeDir}`);
    if (legacyDir) console.log(`✓ Mirrored to ${legacyDir}`);
  } catch (err) {
    console.error('Failed to copy base-set cards:', err.message);
    process.exit(1);
  }
})();
