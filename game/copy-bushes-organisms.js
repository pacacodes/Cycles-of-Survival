#!/usr/bin/env node
/**
 * Copies bushes organism card PNGs from the working organisms output folder
...
function parseArgs() {
  const opts = {
    ...
    categoryConfig: 'game/config/organisms.bushes.json',
    workingDir: 'output/Working Organism Cards/current',
    outDir: 'output/Working Organism Cards/Bushes',
    safeDir: 'saved_files/Working Organism Cards/Bushes',
    legacyDir: 'Output/Bushes',
  };
...
function main() {
  ...
  console.log(`✅ Copied ${copiedCount}/${totalCategoryOrgs} bushes organism cards`);
  if (missingCount > 0) {
